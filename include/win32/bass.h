...
#ifndef BASSDEF
#define BASSDEF(f) __declspec(dllimport _stdcall) f
#define BASSDEFPTR(f) __declspec(dllimport _stdcall) * f
#else
...
const void BASSDEFPTR(BASS_GetConfigPtr)(DWORD option);
...
#if defined(_WIN32) && !defined(_WIN32_WCE) && !(defined(WINAPI_FAMILY) && WINAPI_FAMILY != WINAPI_FAMILY_DESKTOP_APP)
void BASSDEFPTR(BASS_GetDSoundObject)(DWORD object);
#endif
...
const BASS_PLUGININFO BASSDEFPTR(BASS_PluginGetInfo)(HPLUGIN handle);
...
const char BASSDEFPTR(BASS_RecordGetInputName)(int input);
...
const char BASSDEFPTR(BASS_ChannelGetTags)(DWORD handle, DWORD tags);
